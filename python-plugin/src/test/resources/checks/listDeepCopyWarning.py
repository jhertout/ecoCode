...
def test(value:list):
    ret=copy.deepcopy(value) # Noncompliant {{Be sure a deep copy is required, using `copy.deepcopy(x)` of `module copy` to perform a simple shallow copy of a list is not energy efficient.}}
    return ret
print(test(it))
