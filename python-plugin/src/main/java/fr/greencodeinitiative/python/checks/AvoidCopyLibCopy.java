...
@Rule(
        key = "EBOT001",
        name = "Developpement",
        description = AvoidCopyLibCopy.MESSAGE_RULE,
        priority = Priority.MAJOR,
        tags = {"bug"})
public class AvoidCopyLibCopy extends PythonSubscriptionCheck {

    private static final List<String> COPY_LIB = Arrays.asList("copy");

    protected static final String MESSAGE_RULE = "Avoid using Lib/copy.copy(x)";
    ...
    private void checkCallExpression(SubscriptionContext context) {
        ...
        if (isUsingModule && TreeHelper.isMethodCalled(expression, MODULE_TO_CHECK, FUNCTION_TO_CHECK, "list")) {
            context.addIssue(expression, AvoidCopyLibCopy.MESSAGE_RULE);
        }
    }
}